...
public class UserService implements CommunityConstant {
    ...
    public Map<String, Object> register(User user) {
        ...
        // 验证邮箱
        int num = userMapper.selectByEmail(user.getEmail());
        if (num > 0) {
        ...
        userMapper.insertUser(user);    // 注入的时候自动生成id，并回填
    ...
    public Map<String, Object> updatePassword(String oldPassword, String newpassword) {
        ...
        if (!user.getPassword().equals(CommunityUtil.md5(oldPassword + user.getSalt()))) {
            map.put("passwordMsg", "旧密码有误，无法修改密码！");
            return map;
        }

        //修改密码
        int result = userMapper.updatePassword(user.getId(), CommunityUtil.md5(newpassword + user.getSalt()));
        if (result == 0) {
            map.put("systemMsg", "服务器异常，无法修改密码！");
            return map;
        }
